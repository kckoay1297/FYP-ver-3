...
<?php
	...
	// Check connection
		...
		$sql = "SELECT * FROM feedback WHERE userid=".$uid." AND confirm=1";
		$res = $con->query($sql);
		if ($res->num_rows > 0) {
			while( $row = mysqli_fetch_array($res)) {
				echo "<a href='view_feedback.php?id=".$uid."&eid=".$row['event_id']."'><button type='btn' class='btn btn-primary btn-lg sharp aluba'><i class='fa fa-newspaper-o' style='font-size:36px'></i><br>".ucwords($row['name'])."</br></button></a>";	
