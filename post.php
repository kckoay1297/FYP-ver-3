...
function startstatetag($txt){
	...
	$words = explode(" ",$txt);
	$conn = new mysqli("localhost", "root", "", "postdata");
  if ($conn->connect_error) {
	...
	for($i=0;$i<$max;$i++){
		...
		$query = "SELECT pos FROM lexicon WHERE word='".mysqli_real_escape_string($conn,$words[$i])."' COLLATE latin1_general_cs;";
...
*/	
  ...
  if ($conn->connect_error) {
	...
	$result = $conn->query($query);
	//for($cnt=0;$cnt<mysqli_num_rows($result);++$cnt)
		while($row = mysqli_fetch_array($result)){
			$lrules[$cnt] = $row['rule'];
			//echo $lrules[$cnt]."<br>";
		}
		
		//$lrules[$cnt] = mysqli_result($result,$cnt);	
